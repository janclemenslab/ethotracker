...
filepath = sys.argv[1]
print(filepath)
root, recname = os.path.split(filepath)
recname = os.path.splitext(recname)[0]
print(root)
print(recname)
# root = '/Volumes/ukme04/#Common/chaining/dat/localhost-20180226_161811'
...
resdir = '/Volumes/ukme04/#Common/chaining/res'
filename = os.path.join(resdir, recname + '.h5')
...
frame_start = 66350
frame_step = 1
frame_numbers = np.uintp(list(range(frame_start, len(vr), frame_step)))
...
# fly = annotate_2wingextension(vr, frame_numbers, centers[:, chamber_number, :, :], frame_offset, flybox_width=30)
...
fly_box_old = [None]*nflies
focal_fly = None
focal_fly_id = np.zeros((len(list(frame_numbers)), 2), dtype=np.int16)-1
for cnt, frame_number in enumerate(frame_numbers):
    ret, frame = vr.read(frame_number)
    frame = fg.crop(frame, np.ravel(chbb[chamber_number+1][:, ::-1]))
    ...
    for fly0 in range(nflies):
        ...
        try:
            ...
            print('problem cropping frame - setting all white')
            flybox = np.zeros((flybox_width*2, flybox_width*2), dtype=np.uint8)
        ...
        if fly_box_old[fly0] is not None:
            plt.subplot(2,nflies+1, fly0+1)
            plt.imshow(fly_box_old[fly0][10:-10,10:-10], cmap='gray')
            plt.axis('off')
            plt.clim([0, 220])
            plt.title(fly0)
    ...
    plt.show()
    plt.pause(0.0001)
    if focal_fly is None:
        input("resize frame and hit enter")
        focal_fly = int(input(f"select fly to track (0-{nflies}):"))
        old_val = None
    else:
        old_val = focal_fly_id[cnt-1, 1]
    try:
        id = int(input(f"frame {cnt}/{len(list(frame_numbers))} - fly is at position: ENTER={old_val} or NUMBER 0-{nflies} ") or old_val)
    except Exception as e:
        print(e)
        id = -2
    focal_fly_id[cnt, 0] = frame_number
    focal_fly_id[cnt, 1] = id
    # save results as txt
    filename = os.path.join(root, recname + '_id_check.txt')
    np.savetxt(filename, focal_fly_id, fmt='%d')
