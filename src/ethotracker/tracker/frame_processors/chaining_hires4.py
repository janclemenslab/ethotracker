...
def init(vr, start_frame, threshold, nflies, file_name, num_bg_frames=100, annotationfilename=None):
    ...
    for cnt, (cx, cy, cr) in enumerate(zip(ann['centerX'], ann['centerY'], ann['radius'])):
        res.chambers = cv2.circle(res.chambers, (int(cx), int(cy)), int(cr), color=[cnt + 1] * 3, thickness=-1)
